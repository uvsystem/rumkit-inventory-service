...
public class StokMasukController {
	...
	public RestMessage simpan(@PathVariable Long barang, @PathVariable Long jumlah, @PathVariable Date tanggal, @PathVariable Time jam) throws ApplicationException, PersistenceException {
		stokService.simpanStokMasuk(barang, jumlah, tanggal, jam);
		return RestMessage.success();
	}

	@RequestMapping(method = RequestMethod.GET, value = "/{awal}/to/{akhir}")
	@ResponseBody
	public ListEntityRestMessage<StokMasuk> get(@PathVariable Date awal, @PathVariable Date akhir) throws ApplicationException, PersistenceException {
		List<StokMasuk> list = stokService.getStokMasuk(awal, akhir);
